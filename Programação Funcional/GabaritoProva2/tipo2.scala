object Main extends App {
  ...
  def processarLogs(logs: List[Log]) = {
    // Contar quantos usuários realizaram a ação "Logout"
    val totalLogouts = logs.count(x => x.acao == "Logout")

    // Listar usuários únicos que realizaram a ação "Login"
    val usuariosLogin = logs
      .filter(x => x.acao == "Login")
      .map(x => x.usuario)
      .distinct
    ...
    println(s"Logouts: $totalLogouts")
    println(s"Usuários que fizeram Login: ${usuariosLogin}")
